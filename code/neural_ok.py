...
class Autoenc(Model):
  def __init__(self):
    ...
    self.encoder = tf.keras.Sequential([
      ...
      tfa.layers.NoisyDense(7,kernel_initializer='lecun_normal', use_bias=True,
                use_factorised=True,
                activity_regularizer=UncorrelatedFeaturesConstraint(7),
                activation='selu'),
      tfa.layers.PoincareNormalize(),

      tfa.layers.NoisyDense(16,kernel_initializer='lecun_normal', use_bias=True,
                use_factorised=True,
                 activity_regularizer=UncorrelatedFeaturesConstraint(16),
                 activation='selu'),
      tfa.layers.PoincareNormalize(),
      tfa.layers.NoisyDense(8,kernel_initializer='lecun_normal', use_bias=True,
                 use_factorised=True,
                 activity_regularizer=UncorrelatedFeaturesConstraint(8),
                 activation='selu'),
      tfa.layers.PoincareNormalize(),
      # tfa.layers.PoincareNormalize(),
      # tfa.layers.NoisyDense(4,kernel_initializer='lecun_normal',activation='selu'),
      # tfa.layers.PoincareNormalize(),
      tfa.layers.NoisyDense(2,kernel_initializer='lecun_normal', use_bias=True,
                use_factorised=True,
                activation='selu'),
      ])
    ...
    self.decoder = tf.keras.Sequential([
      tf.keras.layers.Dense(8,kernel_initializer='lecun_normal', use_bias=False,
                 activation='selu'),
      tf.keras.layers.Dense(16,kernel_initializer='lecun_normal', use_bias=False,
                 activation='selu'),      
...
class CustomMSE(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        mse = tf.math.reduce_mean((y_true - y_pred)**2)
        mae = tf.math.reduce_mean(tf.abs((y_true - y_pred)))
        r1 = tf.sort(y_true,axis=-1)
        r2 = tf.sort(y_pred,axis=-1)
        r3 = (r1-r2)**2
        l12 = tf.math.reduce_mean(r3)
        l23 = tfa.losses.SigmoidFocalCrossEntropy()
        return 30*mse + 30*l12 + 30*mae + 10*tf.keras.losses.log_cosh(y_true, y_pred)
...
km = MiniBatchKMeans(n_clusters=20, init='k-means++', n_init=100,
                         init_size=30000, batch_size=1024, verbose=2, max_iter=600)
