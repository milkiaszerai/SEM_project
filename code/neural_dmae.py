...
transformer_ica =FastICA(n_components=1)

ica_fit = transformer_ica.fit_transform(img4_re)

# km = MiniBatchKMeans(n_clusters=8, init='k-means++', n_init=100,
#                          init_size=5000, batch_size=1024, verbose=2, max_iter=2500)
...
mm = np.reshape(1000*ica_fit,[768,1024])
mm = (2+2*mm) **2
plt.imshow(mm)
lp = np.reshape(mm,[768,1024])
lp = skimage.morphology.opening(lp,disk(1))
plt.imshow(lp)